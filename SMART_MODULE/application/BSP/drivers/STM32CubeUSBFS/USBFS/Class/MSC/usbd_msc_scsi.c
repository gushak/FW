...
static int8_t SCSI_TestUnitReady(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  if (hmsc->cbw.dDataLength != 0)
  {
	...
	if (((USBD_StorageTypeDef *)pdev->pClassItfMSC)->IsReady(lun) != 0)
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t  SCSI_Inquiry(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  else
  {
	  ...
	  pPage = (uint8_t *)&((USBD_StorageTypeDef *)pdev->pClassItfMSC)->pInquiry[lun * STANDARD_INQUIRY_DATA_LEN];
    len = pPage[4] + 5;
...
static int8_t SCSI_ReadCapacity10(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
	...
	if (((USBD_StorageTypeDef *)pdev->pClassItfMSC)->GetCapacity(lun, &hmsc->scsi_blk_nbr, &hmsc->scsi_blk_size) != 0)
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t SCSI_ReadFormatCapacity(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  for(i=0 ; i < 12 ; i++) 
  {
	...
	if (((USBD_StorageTypeDef *)pdev->pClassItfMSC)->GetCapacity(lun, &blk_nbr, &blk_size) != 0)
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t SCSI_Write10 (USBD_HandleTypeDef  *pdev, uint8_t lun , uint8_t *params)
{
  ...
  if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
  {
    ...
    /* Check If media is write-protected */
	  if (((USBD_StorageTypeDef *)pdev->pClassItfMSC)->IsWriteProtected(lun) != 0)
    {
      SCSI_SenseCode(pdev,
                     lun,
                     NOT_READY, 
...
static int8_t SCSI_ProcessRead (USBD_HandleTypeDef  *pdev, uint8_t lun)
{
  ...
  len = MIN(hmsc->scsi_blk_len , MSC_MEDIA_PACKET); 
	...
	if (((USBD_StorageTypeDef *)pdev->pClassItfMSC)->Read(lun,
                              hmsc->bot_data, 
                              hmsc->scsi_blk_addr / hmsc->scsi_blk_size, 
...
static int8_t SCSI_ProcessWrite (USBD_HandleTypeDef  *pdev, uint8_t lun)
{
  ...
  len = MIN(hmsc->scsi_blk_len , MSC_MEDIA_PACKET); 
	...
	if (((USBD_StorageTypeDef *)pdev->pClassItfMSC)->Write(lun,
                              hmsc->bot_data, 
                              hmsc->scsi_blk_addr / hmsc->scsi_blk_size, 
                              len / hmsc->scsi_blk_size) < 0)
  {
