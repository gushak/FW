                               ...
                               uint8_t cfgidx)
{
  ...
  pdev->pClassDataCDC = &cdcInstance;  
	...
	((USBD_CDC_ItfTypeDef *)pdev->pClassItfCDC)->Init();
                                 ...
                                 uint8_t cfgidx)
{
  ...
  {
	  ((USBD_CDC_ItfTypeDef *)pdev->pClassItfCDC)->DeInit();
                                ...
                                USBD_SetupReqTypedef *req)
{
  ...
  case USB_REQ_TYPE_CLASS :
    if (req->wLength)
    {
      ...
      {
	      ((USBD_CDC_ItfTypeDef *)pdev->pClassItfCDC)->Control(req->bRequest,
                                                          (uint8_t *)hcdc->data,
                                                          req->wLength);
...
{      
  ...
  /* Get the received data length */
	uint32_t size = USBD_LL_GetRxDataSize(pdev, epnum);
  
  /* USB data will be immediately processed, this allow next USB traffic being 
  NAKed till the end of the application Xfer */
  if(pdev->pClassDataCDC != NULL)
  {
	  ((USBD_CDC_ItfTypeDef *)pdev->pClassItfCDC)->Receive(hcdc->Rx.Buffer, &size);
...
{ 
  ...
  if((pdev->pClassItfCDC != NULL) && (hcdc->CmdOpCode != 0xFF))
  {
	  ((USBD_CDC_ItfTypeDef *)pdev->pClassItfCDC)->Control(hcdc->CmdOpCode,
                                                      (uint8_t *)hcdc->data,
                                                      hcdc->CmdLength);
      hcdc->CmdOpCode = 0xFF; 
                                ...
                                uint16_t length)
{
  ...
  hcdc->Tx.Buffer = pbuff;
  hcdc->Tx._s = length;
  hcdc->Tx._w = hcdc->Tx._r = 0;
  return USBD_OK;  
}
...
/**
  ...
  * @retval status
  */
uint8_t  USBD_CDC_TransmitPacket(USBD_HandleTypeDef *pdev, int size)
{      
	USBD_CDC_HandleTypeDef   *hcdc = (USBD_CDC_HandleTypeDef *)pdev->pClassDataCDC;
...
int _inc(USBD_CDC_HandleTypeDef *hcdc, int i, int n)
{
	i += n;
	if (i >= hcdc->Rx._s)
		i -= hcdc->Rx._s;
