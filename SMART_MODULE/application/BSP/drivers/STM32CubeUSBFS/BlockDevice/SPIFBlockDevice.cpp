...
int SPIFBlockDevice::init()
{
	...
	if (ADDR_LEN == 4)
	{
		_cmdwrite(ENTER_4ADDR_MODE, 0, 0, 0x0, NULL);
	}
    ...
    // specific info
    uint8_t header[16];
	_cmdread(SPIF_SFDP, ADDR_LEN, 16, 0x0, header);
    ...
    uint32_t table_addr = (
        ...
        (header[12] << 8 ));
    uint8_t table[8];
	_cmdread(SPIF_SFDP, ADDR_LEN, 8, table_addr, table);
        ...
        uint32_t addr, uint8_t *rets)
{
	...
	uint8_t dummyByte;
	if (retc == PAGE_SIZE)
		spi_dma_read(_spi_id, &dummyByte, rets, retc);
	else
#endif
        ...
        uint32_t addr, const uint8_t *args)
{
	...
	uint8_t dummyByte;
	if (argc == PAGE_SIZE)
		spi_dma_write(_spi_id, (uint8_t *)args, &dummyByte, argc);
	else
#endif
...
int SPIFBlockDevice::read(void *buffer, bd_addr_t addr, bd_size_t size)
{
	...
	_cmdread(SPIF_READ, ADDR_LEN, size, addr, static_cast<uint8_t *>(buffer));
    return 0;
}
...
int SPIFBlockDevice::program(const void *buffer, bd_addr_t addr, bd_size_t size)
{
    ...
    while (size > 0) {
	    ...
	    _cmdwrite(SPIF_PROG, ADDR_LEN, chunk, addr, static_cast<const uint8_t *>(buffer));
...
uint8_t SPIFBlockDevice::_statusflag()
{
	uint8_t status;
	_cmdread(WRITE_STATUS_REG, 0, 1, 0x0, &status);
	return status;
}

void SPIFBlockDevice::_clear()
{
	_cmdwrite(CLEAR_FLAG_STATUS_REG, 0, 0, 0, NULL);
	wait_us(1);
	uint8_t data = 0x00;
	_cmdwrite(WRITE_STATUS_REG, 0, 1, 0, &data);
}

int SPIFBlockDevice::_erase() 
{
	int timeout = SPIF_TIMEOUT;

	for (uint8_t i = 0; i < TOTAL_DIES; i++) {
		_clear();
		while (_statusflag() && timeout--)
			wait_us(1);
		if (timeout <= 0)
			return BD_ERROR_DEVICE_ERROR;
		...
		_cmdwrite(WRITE_EXT_ADDR_REG, 0, 1, 0, &i);
		
		err = _wren();
		if (err) {
			return err;
		}
		_cmdwrite(DIE_ERASE, 0, 0, 0, NULL);
...
int SPIFBlockDevice::_erase64k(bd_addr_t addr, bd_size_t size)
{
	int timeout = SPIF_TIMEOUT;
	uint32_t chunk = SECTOR_64K_SIZE;
	...
	while (size > 0) 
	{
		...
		_cmdwrite(SECTOR_ERASE64K, ADDR_LEN, 0, addr, NULL);
		addr += chunk;
		size -= chunk;
...
int SPIFBlockDevice::erase(bd_addr_t addr, bd_size_t size)
{
    ...
    while (size > 0) {
        ...
        uint32_t chunk = 4096;
	    _cmdwrite(SPIF_SE, ADDR_LEN, 0, addr, NULL);
        addr += chunk;
        size -= chunk;
...
int SPIFBlockDevice::disk_initialize() {
...
int SPIFBlockDevice::disk_write(const uint8_t * buffer, uint64_t sector, uint8_t count = 1) {
	bd_addr_t _addr = sector << 9;
	bd_size_t _size = 512*count;
	
	return program(buffer, _addr, _size);
}

int SPIFBlockDevice::disk_read(uint8_t * buffer, uint64_t sector, uint8_t count = 1) {
	bd_addr_t _addr = sector << 9;
	bd_size_t _size = 512*count;

	return read(buffer, _addr, _size);
}

int SPIFBlockDevice::_disk_status() { return _status; }
int SPIFBlockDevice::disk_sync() { 
	return _sync();
}
uint64_t SPIFBlockDevice::disk_sectors() { return _sectors; }

int SPIFBlockDevice::disk_erase(uint64_t sector, uint8_t count = 1) {
	if (sector % 8 || count % 8)
		return -1;
	
	bd_addr_t _addr = sector << 8;
	bd_size_t _size = SPIF_SE_SIZE*count;
	return erase(_addr, _size);
}

int SPIFBlockDevice::disk_erase() {
	int err = _wren();
	_cmdwrite(SPIF_CE, 0, 0, 0x0, NULL);
...
//bool SPIFBlockDevice::localSafe(void) {
//	return (local_count == 0);
//}
//
//bool SPIFBlockDevice::usbSafe(void) {
//	return usbfree;
//}

int SPIFBlockDevice::disk_status_msd() { 
