...
#ifdef CHIP_ERASE_FEATURE
	u32_t phys_addr = fs->cfg.phys_addr;
	u32_t log_block_size = fs->cfg.log_block_size;	
	fs->cfg.phys_addr = 0;
	fs->cfg.log_block_size = 0;
    res = spiffs_erase_block(fs, 0);
	if (res != SPIFFS_OK) {
		res = SPIFFS_ERR_ERASE_FAIL;
	}
	fs->cfg.phys_addr = phys_addr;
	fs->cfg.log_block_size = log_block_size;
