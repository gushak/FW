...
#define AUTOMAN_BAHAP_DATA_SIZE				0x0D
#define AUTOMAN_BAHAP_EX_SIZE				0x08
#define AUTOMAN_BAHAP_INFO_SIZE				0x03


#define RF_MAIN_CMD_BAHAP_REQ              0x00
#define RF_MAIN_CMD_BAHAP_CHANGE           0x01

#define RF_SUB_CMD_BAHAP_REQ               0x00
#define RF_SUB_CMD_BAHAP_CHANGE            0x01

#define RF_SUB_CMD_BAHAP_REQ_DATA               0x01
#define RF_SUB_CMD_BAHAP_CHANGE_DATA            0x02
...
typedef struct 
{
	...
	uint16_t RelayBack;                   // relay value	        
	uint8_t InEvent;
	uint8_t RelayEvent;  
        uint8_t Len;                         // 데이터 총길이 
	uint16_t Fcode;			// 농장코드 
...
typedef struct LCD_ICON
{
        uint32_t current_page;
        uint16_t iconbuffer[10];
}Lcdicon_st;
...
extern uint8_t PorcessPacket_Rf_check(uint8_t rx_data, uint8_t* buffdata, uint8_t mode);
uint8_t PorcessPacket_Rf_check_Behap(uint8_t rx_data, uint8_t* buffdata, uint8_t mode);
extern uint8_t RfRxCheckCount;
...
extern void Set_Page16(uint16_t data);
