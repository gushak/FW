...
uint16_t make_crc16(const void *pbuffer, size_t size)
{
	...
	if (size > 0)
	{
		while (size--)
		{
			fcs = (uint16_t)(fcs >> 8) ^ fcstab_crc[(fcs ^ *(p++)) & 0xff];
		}
...
{
	uint16_t crc = make_crc16(data, data_len);

	memcpy(data + data_len, &crc, 2);
}

const uint32_t fcstab32[256] = {
...
uint32_t make_crc32(const void* pbuffer, size_t size)
{
    ...
    while (size--)
    {
        fcs = (((fcs >> 8) & 0x00FFFFFF) ^ fcstab32[(fcs ^ *p++) & 0x000000FF]);
        //fcs = fcstab[(fcs ^ *p++) & 0xFF] ^ (fcs >> 8);
    }
    fcs ^= PPPINITFCS32;
